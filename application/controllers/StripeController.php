...
class StripeController extends CI_Controller {
    ...
    */
    ...
    public function stripeSub()
    {
        ...
        \Stripe\Stripe::setApiKey($this->config->item('stripe_secret'));
        $cId=0;
        $customer=\Stripe\Customer::all(["email" => $this->input->post('email')]);
        ...
        try {
            ...
            'customer' => $cId,
            'items' => [['plan' => $this->input->post('palnId')]],
            'metadata' => array(
              "URL"=>base_url("campaign/detail/".$pid),
              "description"=>$this->input->post('description'))
        ]);
          // Used Stripe's library to make requests...
...
*/
        ...
        if ($msg!="") {
            ...
            $this->session->set_flashdata('amount', 100);
